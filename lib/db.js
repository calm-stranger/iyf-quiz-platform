...
async function resetAttempt(attemptId) {
  ...
  const rows = await request('attempts', `id=eq.${attemptId}`, {
    ...
    body: JSON.stringify({
      ...
      submitted_at: null,
      auto_submitted: false,
      // Violations must go too. Auto-submit fires at two, so a student reset
      // after being auto-submitted would carry those two straight back in and
      // be thrown out again on their first blur — the reset would look broken.
      violations: 0,
    }),
  });
  return rows?.[0] || null;
...
async function getWrongAnswers(attemptId) {
  ...
  const { shuffleWithSeed, optionIndexList } = require('./shuffle');
  ...
  return answers.map((a) => {
    ...
    // Must match start.js exactly, including for questions with fewer than
    // four options, or the replayed order will not be what the student saw.
